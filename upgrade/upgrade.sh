#!/bin/sh -xe
# k3s-void-upgrade — k3s upgrade script adapted for Void Linux
# Based on /bin/upgrade.sh from rancher/k3s-upgrade:v1.36.4-k3s1 (extracted 2026-08-30).
#
# Void adaptations (marked [VOID]):
#   1. verify_host_mount()  — hostPath mounts are UNSTABLE on k3s-v3 (Void): pods can
#      silently get an overlay of the container rootfs at /host. Without this check the
#      "new" binary gets written to the container, not the host, and the node stays on
#      the old version. Abort before touching anything.
#   2. ensure_new_binary()  — the SUC normally stages the new binary at /opt/k3s.
#      Fallback: resolve the latest release dynamically via the GitHub API (no pinned
#      version) and download it straight to /opt/k3s.
#   3. kill_k3s_process()   — original SIGTERM + supervisor respawn (supervise-daemon
#      on Void respawns with respawn-max 0). Added post-kill verification and an
#      rc-service fallback.
#
# The original script is already systemd-agnostic: it SIGTERMs the k3s process and
# lets the host supervisor respawn it, and explicitly handles supervise-daemon parents
# in get_k3s_process_info().
...
# [VOID] Guard against the broken hostPath mount seen on k3s-v3 (Void nodes).
# When the mount fails, /host is an overlay of the container rootfs (Ubuntu) instead
# of the host root (Void). Never write a binary through a mount that is not the host.
verify_host_mount() {
  if [ ! -f /host/etc/os-release ]; then
    fatal "No /host/etc/os-release — host mount missing, aborting"
  fi
  if ! grep -qi 'NAME="Void"' /host/etc/os-release; then
    fatal "Host mount not valid (got: $(grep -E '^(NAME|PRETTY_NAME)=' /host/etc/os-release | head -1)) — aborting before touching anything"
  fi
}
...
# [VOID] Fallback when the SUC did not stage a binary at /opt/k3s: resolve the latest
# release dynamically (nothing pinned) and download it.
ensure_new_binary() {
  if [ -f /opt/k3s ]; then
    return
  fi
  info "No staged binary at /opt/k3s; resolving latest k3s release"
  VER=$(curl -fsSL https://api.github.com/repos/k3s-io/k3s/releases/latest | grep -oP '"tag_name":\s*"\K[^"]+')
  ARCH=$(uname -m | sed 's/aarch64/arm64/; s/x86_64/amd64/')
  info "Downloading k3s ${VER} (${ARCH})"
  curl -fsSL -o /opt/k3s "https://github.com/k3s-io/k3s/releases/download/${VER}/k3s-${ARCH}"
  chmod +x /opt/k3s
}
...
kill_k3s_process() {
    ...
    # (supervise-daemon on Void, systemd elsewhere) to automatically restart k3s
    # with the new version
    kill -SIGTERM "$K3S_PID"
    info "Successfully killed old k3s pid $K3S_PID"

    # [VOID] Post-kill verification: supervise-daemon respawns with respawn-delay 5.
    # Give it time, then confirm a NEW k3s process exists (different pid). If nothing
    # came back, try rc-service as a fallback (present on some Void/openrc hosts).
    sleep 20
